...
NAME_RE = re.compile('^[A-Za-z0-9_\\-]{1,64}$')
...
def _run_job(strategy_id: int, tickers: List[str], test_types: List[str], depth: str, date_from=None, date_to=None) -> None:
    try:
        ...
        for i, tk in enumerate(tickers, 1):
            ...
            if 'full_sample' in test_types:
                ...
                if r.get('status') == 'success':
                    ...
                    m = {'error': r.get('error')}
                db.execute('INSERT INTO trading.backtest_results (strategy_id, ticker, test_type, depth, metrics) VALUES (%s,%s,%s,%s,%s)', (strategy_id, tk, 'full_sample', depth, _json_dumps(m)))
            else:
                pass
            if 'walkforward' in test_types and (not custom_period):
                wf = run_walkforward(db, tk, config)
                db.execute('INSERT INTO trading.backtest_results (strategy_id, ticker, test_type, depth, metrics) VALUES (%s,%s,%s,%s,%s)', (strategy_id, tk, 'walkforward', depth, _json_dumps(wf)))
            else:
                pass
            done = i
        ...
        _write_report({'status': 'done', 'error': None, 'finished_at': datetime.now().isoformat(), 'tickers_done': done, 'tickers_total': len(tickers)})
    except Exception as exc:
        jobs_state.finish(JOB, status='failed', error=str(exc))
        _write_report({'status': 'failed', 'error': str(exc), 'traceback': traceback.format_exc(), 'finished_at': datetime.now().isoformat()})
        traceback.print_exc()
    else:
        pass
    finally:
...
def register_routes(app: FastAPI) -> None:
    ...
    def data_range():
        ...
        if df.empty or df.iloc[0]['mn'] is None:
            ...
            pass
        return {'min_date': pd.Timestamp(df.iloc[0]['mn']).strftime('%Y-%m-%d'), 'max_date': pd.Timestamp(df.iloc[0]['mx']).strftime('%Y-%m-%d')}
    ...
    def run_strategy(strategy_id: int, payload: RunIn):
        ...
        for tt in payload.test_types:
            ...
            pass
        if not jobs_state.try_start(JOB, stage='starting', strategy_id=strategy_id, tickers_total=len(payload.tickers), tickers_done=0):
            raise HTTPException(status_code=409, detail={'message': 'A heavy job is already running', 'jobs': jobs_state.all_snapshots()})
        else:
            pass
        _write_report({'strategy_id': strategy_id, 'tickers': payload.tickers, 'test_types': payload.test_types, 'depth': payload.depth, 'started_at': datetime.now().isoformat(), 'status': 'starting', 'error': None})
        run_params = {'tickers': payload.tickers, 'test_types': payload.test_types, 'depth': payload.depth, 'date_from': payload.date_from, 'date_to': payload.date_to}
        db = _get_db()
        db.execute("UPDATE trading.strategies SET config = coalesce(config, '{}'::jsonb) || CAST(%s AS jsonb) WHERE id=%s", (_json_dumps({'run_params': run_params}), strategy_id))
        threading.Thread(target=_run_job, args=(strategy_id, payload.tickers, payload.test_types, payload.depth, payload.date_from, payload.date_to), name='strategy-backtest', daemon=True).start()
    ...
    def strategy_results(strategy_id: int):
        db = _get_db()
        df = db.select('SELECT id, ticker, test_type, depth, metrics, created_at::text AS created_at FROM trading.backtest_results WHERE strategy_id=%s ORDER BY test_type, ticker', (strategy_id,)).to_dataframe()
        rows = df.to_dict('records')
        for r in rows:
