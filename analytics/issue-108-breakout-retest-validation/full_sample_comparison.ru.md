# Задача #108: full-sample сравнение A и B

База **A** — Lab-конфиг `test_20260820` (id=102) после вето #97, та же вселенная и окно, что в задаче #100. **B** — тот же конфиг плюс `level_breakout_retest` (дефолты Lab). Locked `test_20260731` не перезаписывался. Упрощённый JSON в issue опускал `signal_4h_buy` и окна подтверждения; для совпадения с #100 они обязательны.
...
Бит-в-бит с задачей #100: **28/28** тикеров (расхождений=0).
