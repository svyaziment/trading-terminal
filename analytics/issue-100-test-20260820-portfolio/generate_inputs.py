"""Generate the portfolio inputs required by Issue #100.

Parallel per-ticker StrategyPlugin backtests for DB strategy id=102
(`test_20260820`), then one portfolio replay with Issue #44 slot rules.
...
Locked test_20260731 (id=36) is not read or written.
"""
...
def _output_path(mode: str) -> Path:
    return REPO_ROOT / "reports/Arctic/100_test-20260820-portfolio" / f"{mode}_run.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Issue #100 inputs")
