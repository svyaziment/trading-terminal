"""Reproducible portfolio analysis for Issue #103 (test_20260821, id=118).
...
DISPLAY_NAME = "test_20260821"
STRATEGY_ID = 118
ANALYSIS_DIR = Path(__file__).resolve().parent
REPO_ROOT = ANALYSIS_DIR.parents[1]
DEFAULT_INPUT = (
    REPO_ROOT / "reports/Arctic/103_test-20260821-portfolio/full_run.json"
)
...
def _load_result(path: Path) -> dict[str, Any]:
    ...
    if int(result.get("strategy_id", 0)) != STRATEGY_ID:
        raise ValueError(
            f"{path}: expected strategy_id={STRATEGY_ID}, got {result.get('strategy_id')!r}"
        )
    if result.get("strategy_config_name") != DISPLAY_NAME:
        raise ValueError(
            f"{path}: expected strategy_config_name={DISPLAY_NAME!r}, "
    ...
    if float(result.get("initial_capital_rub", 0)) != 50_000.0:
        raise ValueError(f"{path}: Issue #103 requires initial capital of 50,000 RUB")
    if float(result.get("slot_size_rub", 0)) != 10_000.0:
        raise ValueError(f"{path}: Issue #103 requires slot size of 10,000 RUB")
    if int(result.get("max_positions", 0)) != 5:
        raise ValueError(f"{path}: Issue #103 requires max_positions=5")
    ...
    levels = _levels_params(result["strategy_config"])
    if list(levels.get("level_method") or []) != ["swing", "impulse"]:
        raise ValueError(
            f"{path}: expected level_method=['swing','impulse'], "
...
def plot_equity(equity: pd.Series) -> None:
    ...
    axis.set_title("Daily realized equity — test_20260821")
...
def build_report(
    ...
    params_line = (
        "Параметры `test_20260821`, `test_20260820` и locked "
    ...
    if paper_ok:
        verdict = (
            ...
            f"{metrics.max_drawdown_pct:.2f}%). {params_line}"
        )
    else:
        verdict = (
            "По правилам #44 это **не кандидат** в paper: нужны положительный PnL, "
            f"PF>1, n≥30 и отсутствие GAME OVER. {params_line}"
        )
    if alrs["present"]:
        alrs_line = (
    ...
    return f"""# Issue #103: портфельный бэктест test_20260821
...
{ISSUE44_CONTEXT['max_drawdown_pct']:.2f}%.
...
Equity {ISSUE100_CONTEXT['final_equity_rub']:,.2f} RUB,
PnL {ISSUE100_CONTEXT['pnl_rub']:+,.2f} RUB ({ISSUE100_CONTEXT['pnl_pct']:+.2f}%),
{ISSUE100_CONTEXT['n_trades']} сделок, WR {ISSUE100_CONTEXT['win_rate_pct']:.1f}%,
PF {ISSUE100_CONTEXT['profit_factor']:.2f}, daily Max DD
{ISSUE100_CONTEXT['max_drawdown_pct']:.2f}%.
...
def run_analysis(input_path: Path | None = None) -> dict[str, Any]:
    ...
    summary = {
        ...
        "issue100_context": ISSUE100_CONTEXT,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Issue #103 portfolio analysis")
    parser.add_argument(
        "--input",
        type=Path,
