"""Build the executable notebook for Issue #103."""
...
def build() -> Path:
    ...
    notebook["cells"] = [
        nbf.v4.new_markdown_cell(
            ...
            "Один конфиг (`trading.strategies.id=118`) на общем портфеле 50,000 RUB. "
        ...
        nbf.v4.new_code_cell(
            ...
            "    ANALYSIS_DIR = Path("
            "'analytics/issue-103-test-20260821-portfolio').resolve()\n"
            "sys.path.insert(0, str(ANALYSIS_DIR))\n"
            "from analysis import run_analysis, DISPLAY_NAME"
        ),
        nbf.v4.new_markdown_cell(
            "## Расчёт\n"
            "`run_analysis()` валидирует id=118, `level_method=['swing','impulse']`, "
            "стартовый капитал, слоты и покрытие 2026-08-20. Daily equity строится "
            "по моментам закрытия сделок (realized equity без mark-to-market). "
            "Бар ALRS paper #711 во входах является блокером."
        ),
