...
TASK_ID="task-021-push-market-data-pr"
ROOT_DIR="$(pwd)"
...
PR_TITLE="feat: market data API endpoints"
PR_BODY="Add market data API endpoints.

Endpoints:
- GET /api/instruments
- GET /api/candles
- GET /api/signals
- GET /api/top-stocks-by-volume

Details:
- reads data from existing trading schema
- direct FastAPI route registration
- backend tests pass
- Docker build includes route assertion"
...
if [ "$STATUS" = "success" ] && [ "$CURRENT_BRANCH" != "$FEATURE_BRANCH" ]; then
  ...
  if git show-ref --verify --quiet "refs/heads/$FEATURE_BRANCH"; then
    ...
    add_error "Run task-020f-register-routes-direct first or checkout $FEATURE_BRANCH."
...
if UNEXPECTED_CHANGES="$(working_tree_acceptable)"; then
  add_check "git_status_acceptable" "working tree" "true" "Clean or only scripts changes"
...
if [ "$STATUS" = "success" ]; then
  echo "Staging task scripts..."

  git add scripts/ 2>/dev/null || true

  add_check "git_add" "scripts/" "true"
  echo "OK: git add scripts completed"
fi
...
if [ "$STATUS" = "success" ]; then
  if git diff --cached --quiet; then
    ...
    if git commit -m "chore(task-021): add market data PR scripts"; then
      COMMIT_CREATED="true"
