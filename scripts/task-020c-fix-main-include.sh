...
TASK_ID="task-020c-fix-main-include"
ROOT_DIR="$(pwd)"
...
echo "Rewriting backend/app/main.py using Python..."
...
print("OK: backend/app/main.py rewritten")
PY_WRITE_MAIN
...
if [ "$STATUS" = "success" ]; then
  ...
  ROUTE_DIAG="$(docker compose run --rm -T backend python - <<'PY_ROUTE_DIAG'
import app.main

routes = sorted({getattr(route, "path", "") for route in app.main.app.routes})
routes_str = ",".join(routes)

print("APP_ROUTES=" + routes_str)
print("HAS_INSTRUMENTS=" + ("true" if "/api/instruments" in routes else "false"))
PY_ROUTE_DIAG
)"
  ...
  if [ "$HAS_INSTRUMENTS" = "true" ]; then
    ...
    add_error "app does not contain /api/instruments after fix."
    echo "FAIL: app does not contain /api/instruments"

    echo "Dumping container main.py with visible characters:"
    docker compose run --rm -T backend sh -lc 'cat -A /app/app/main.py' || true
  fi
fi
...
if [ "$STATUS" = "success" ]; then
  ...
  git add backend/tests/test_routes.py 2>/dev/null || true
...
if [ "$STATUS" = "success" ]; then
  if git diff --cached --quiet; then
    ...
    if git commit -m "fix(task-020): ensure market data router is included at module level"; then
      COMMIT_CREATED="true"
