...
TASK_ID="task-025-indicators-manager"
ROOT_DIR="$(pwd)"
...
# ---------- Создаём analytics/__init__.py ----------
mkdir -p backend/app/analytics
touch backend/app/analytics/__init__.py

# ---------- Создаём indicators_manager.py ----------
cat > backend/app/analytics/indicators_manager.py <<'PYEOF'
"""
Indicators Manager.
...
echo "OK: indicators_manager.py created"
...
# ---------- Run indicators for VTBR ----------
echo "Calculating indicators for VTBR..."

INDICATORS_OUTPUT="$(docker compose run --rm -T --no-deps backend \
    python -m app.analytics.indicators_manager \
    --ticker VTBR \
    --timeframes 30min,1h,4h,1d,1w,1M \
    2>&1)" || true
...
INDICATORS_TOTAL="$(echo "$INDICATORS_OUTPUT" | grep -oP 'INDICATORS_TOTAL_SUCCESS=\K\d+' || echo "0")"
echo "INDICATORS_TOTAL_SUCCESS=${INDICATORS_TOTAL}"

# ---------- Commit ----------
echo "Staging files..."
git add backend/app/analytics/ scripts/task-025-indicators-manager.sh 2>/dev/null || true

if ! git diff --cached --quiet; then
    git commit -m "feat(task-025): add indicators manager"
...
cat > "$REPORT_JSON" <<EOF
{
  ...
  "checks": [
    {"name": "indicators_manager_created", "status": "passed"},
    {"name": "docker_build", "status": "passed"},
    {"name": "indicators_calculated", "status": "passed", "count": "${INDICATORS_TOTAL}"},
...
**Branch:** $(git rev-parse --abbrev-ref HEAD)
**Commit:** ${COMMIT_SHA}
**Indicators calculated:** ${INDICATORS_TOTAL}
EOF
