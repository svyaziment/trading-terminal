...
write_report() {
  ...
  export ORIGINAL_BRANCH ACTIVE_FEATURE FINAL_BRANCH FEATURE_BRANCH NEXT_BRANCH BASE_BRANCH
...
r = {
    ...
    "active_feature_branch": os.environ["ACTIVE_FEATURE"],
    ...
    "next_action": "Send this report.json. If pr_url empty and status=needs_human, run: gh auth login (or open the new-PR link from log.txt)."
}
...
md = [
    ...
    "Active feature branch: " + r["active_feature_branch"],
    ...
    cat > "${REPORT_JSON}" <<JSON
{
  ...
  "active_feature_branch": "${ACTIVE_FEATURE}",
...
fail() {
  ...
  log "ERROR [$1]: $2"
  FINAL_BRANCH="$(git branch --show-current 2>/dev/null || echo "${ACTIVE_FEATURE}")"
  write_report
  exit 0
}
...
# Подавляем безвредные advice-сообщения (чище лог)
...
if [[ -z "$(git config user.name 2>/dev/null || echo "")" ]]; then
  git config user.name "Trading Terminal Bot"; log "Set local user.name"
fi
if [[ -z "$(git config user.email 2>/dev/null || echo "")" ]]; then
  git config user.email "trading-terminal@local"; log "Set local user.email"
fi
...
add_ignore ".DS_Store"
# reports/ игнорируем ТОЛЬКО если не просили их коммитить
...
# Умный выбор feature-ветки: не плодим дубли после неудачного запуска.
if [[ "${ORIGINAL_BRANCH}" == "${BASE_BRANCH}" ]]; then
  ...
  log "On base branch, creating feature branch ${FEATURE_BRANCH}"
  git checkout -b "${FEATURE_BRANCH}" >>"${LOG_TXT}" 2>&1 || fail "checkout_feature" "Cannot create/checkout ${FEATURE_BRANCH}"
  ACTIVE_FEATURE="${FEATURE_BRANCH}"
else
  log "Already on non-base branch ${ORIGINAL_BRANCH}, reusing it as feature branch"
  ACTIVE_FEATURE="${ORIGINAL_BRANCH}"
fi

log "Staging files with plain 'git add -A' (relying on .gitignore)"
if ! git add -A >>"${LOG_TXT}" 2>&1; then
  fail "git_add" "git add -A failed. See log.txt."
fi
...
if git diff --cached --quiet 2>/dev/null; then
  ...
  ERROR_MESSAGE="No staged changes to commit. Working tree may already be clean on ${ACTIVE_FEATURE}."
  FINAL_BRANCH="${ACTIVE_FEATURE}"
  write_report; exit 0
fi
...
log "Pushing ${ACTIVE_FEATURE}"
git push -u origin "${ACTIVE_FEATURE}" >>"${LOG_TXT}" 2>&1 || fail "git_push" "git push failed for ${ACTIVE_FEATURE}"
...
.env и секреты, frontend/node_modules, frontend/dist, reports/ (по умолчанию), logs/, сертификаты backend/certs/*.pem.

## Проверка

- npm install проходил успешно (task-032a).
- Frontend проверен визуально в браузере (фильтры, сортировка, пагинация, sticky-шапка).
- Полная верификация backend API совместимости с фронтом запланирована следующим шагом (Block A verify).
...
- Диагностика отсутствия 1d-сигналов.

## Changed files (vs base)
PRBODY
cat "${CHANGED_LIST_FILE}" >> "${PR_BODY_FILE}" 2>/dev/null || true

PR_URL="$(gh pr list --head "${ACTIVE_FEATURE}" --state open --json url --jq '.[0].url' 2>>"${LOG_TXT}" || echo "")"
...
if [[ -n "${PR_URL}" ]]; then
  ...
  if ! command -v gh >/dev/null 2>&1; then
    ...
    PR_URL="$(gh pr create --base "${BASE_BRANCH}" --head "${ACTIVE_FEATURE}" \
      --title "${COMMIT_MESSAGE}" --body-file "${PR_BODY_FILE}" 2>>"${LOG_TXT}" || echo "")"
    if [[ -n "${PR_URL}" ]]; then log "Pull request created: ${PR_URL}";
...
[[ -n "${FINAL_BRANCH}" ]] || FINAL_BRANCH="${ACTIVE_FEATURE}"
