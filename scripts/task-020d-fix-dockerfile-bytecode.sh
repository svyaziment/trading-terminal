...
TASK_ID="task-020d-fix-dockerfile-bytecode"
ROOT_DIR="$(pwd)"
...
echo "Converting Python files to LF..."
...
do
  if [ -f "$f" ]; then
    sed -i 's/\r$//' "$f"
    echo "OK: converted LF: $f"
  fi
done

add_check "convert_lf" "backend python files" "true"
echo "OK: LF conversion step completed"

echo "Updating backend/.dockerignore..."

touch backend/.dockerignore

if ! grep -qxF '**/__pycache__' backend/.dockerignore; then
  echo '**/__pycache__' >> backend/.dockerignore
  echo "OK: added **/__pycache__ to backend/.dockerignore"
fi

if ! grep -qxF '**/*.pyc' backend/.dockerignore; then
  echo '**/*.pyc' >> backend/.dockerignore
  echo "OK: added **/*.pyc to backend/.dockerignore"
fi

add_check "dockerignore_updated" "backend/.dockerignore" "true"
...
RUN python -c "import app.main; routes={getattr(route, 'path', '') for route in app.main.app.routes}; assert '/api/instruments' in routes, 'Missing /api/instruments: ' + str(sorted(routes))"
...
if [ "$STATUS" = "success" ]; then
  ...
  if docker compose build --no-cache backend; then
    ...
    add_error "Docker build failed. Likely route assertion failed. See log."
...
if [ "$STATUS" = "success" ]; then
  ...
  if ROUTE_DIAG="$(docker compose run --rm -T backend python -c "import app.main; routes=sorted({getattr(route, 'path', '') for route in app.main.app.routes}); print('APP_ROUTES=' + ','.join(routes)); print('HAS_INSTRUMENTS=' + ('true' if '/api/instruments' in routes else 'false'))" 2>&1)"; then
    ...
    if [ "$HAS_INSTRUMENTS" = "true" ]; then
      ...
      add_error "app does not contain /api/instruments after Dockerfile fix."
...
if [ "$STATUS" = "success" ]; then
  ...
  git add backend/Dockerfile backend/.dockerignore backend/app/main.py backend/app/api/__init__.py backend/app/api/market_data.py backend/tests/test_routes.py scripts/ 2>/dev/null || true
...
if [ "$STATUS" = "success" ]; then
  if git diff --cached --quiet; then
    ...
    if git commit -m "fix(task-020): force LF, exclude pycache, assert API routes during docker build"; then
      COMMIT_CREATED="true"
