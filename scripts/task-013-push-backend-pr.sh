...
TASK_ID="task-013-push-backend-pr"
ROOT_DIR="$(pwd)"
...
PR_TITLE="feat: backend base with health endpoint"
PR_BODY="Add backend base service with FastAPI health endpoint and Docker tests.
...
if [ "$CURRENT_BRANCH" != "$FEATURE_BRANCH" ]; then
  if [ -n "$(git status --porcelain)" ]; then
    add_check "git_branch" "$FEATURE_BRANCH" "false" "Not on feature branch and working tree is not clean" "needs_human"
    add_error "Checkout $FEATURE_BRANCH manually or clean working tree."
    echo "FAIL: not on feature branch and working tree is not clean"
  else
    if git show-ref --verify --quiet "refs/heads/$FEATURE_BRANCH"; then
      if git checkout "$FEATURE_BRANCH"; then
        CURRENT_BRANCH="$FEATURE_BRANCH"
        add_check "git_branch" "$FEATURE_BRANCH" "true" "Switched to existing feature branch"
        echo "OK: switched to $FEATURE_BRANCH"
      else
        add_check "git_branch" "$FEATURE_BRANCH" "false" "Cannot checkout feature branch"
      ...
      add_error "Run task-012a-backend-branch-fix first."
...
if [ -n "$(git status --porcelain)" ]; then
  ...
  add_error "Working tree is not clean. Commit or stash changes before pushing."
