...
if [ -d ".git" ]; then
  ...
  add_check "git_branch" "$GIT_BRANCH" "true"
  echo "OK: current branch is $GIT_BRANCH"
fi
...
if [ "$STATUS" = "success" ] && [ "$MERGED" = "true" ]; then
  ...
  if [ -n "$(git status --porcelain)" ]; then
    ...
    add_error "Working tree is not clean. Commit or stash changes before syncing main."
