...
- **Рантайм**: исполнитель пока не продвигает лестницу бар за баром — это задача #151. Пока
  она не легла, каждая live-позиция отдаёт `trailing_enabled=false` и
  `current_stop_price` / `step_reached` / `risk_r` как `null`, так что API обратно совместим,
  а UI видит стабильную форму.
- **Чтение прогона**: `exit_reason='trailing'` у live-позиции означает, что проводка легла
  (#151) и лестница сработала; до этого выходы только `stop` или `take`.
...
Полный набор параметров и форматы отчётов — в `testing-rules.ru.md`.
  ...
  - `closed_trailing` — та же форма, что у `closed_stop`, но стоп был поджат трейлинг-лестницей
    перед срабатыванием. **Готова схема** (задача #149): колонки есть в
    `trading.live_positions` через миграцию `20260915_002_live_trailing.py`, но исполнитель их
    пока не ведёт — проводка за задачей #151.
