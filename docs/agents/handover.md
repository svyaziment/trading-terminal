...
Last refreshed: 2026-08-19 (Issue #81 SignalEngine Lab E2E). Companion to project-context.md.
...
- How to enable in the constructor: add a SignalEngine chip from `GET /api/patterns` (do not hardcode the ten ids in `StrategyLab.tsx`). Timeframe and pattern params are set in `PatternSettingsModal`. Save runs `normalize_patterns`; the same config is used by `strategy_backtest`, paper (`get_active_paper_strategy` → `StrategyEvaluator`), and live. Do not overwrite locked `test_20260731`.
