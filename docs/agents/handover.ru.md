...
Последнее обновление: 2026-09-07 (задача #143 — решётка v3 из #155: 8 сеток, `summary.json.lattice` и тесты `backend/tests/test_issue155_analysis.py`, §35). Сопутствующий файл: `project-context.ru.md` (английский оригинал: `project-context.md`).
...
- Вывод: только аналитика. Composite-балл устойчивости не автономен: компоненты стресс-капитала,
  ...
  PO-пробу `ultra_late_tight` утверждает Product Owner (#144); работа над движком — #145, паритет
  боевого пути — #147, песочница — #151, приёмка — #152. Статус зеркалится в `project-context.ru.md` §18 и
  блок W дорожной карты (§8).
