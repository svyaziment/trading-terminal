...
Последнее обновление: 2026-08-19 (задача #81 E2E SignalEngine Lab; синхронизировано с английской версией). Сопутствующий файл: `project-context.ru.md` (английский оригинал: `project-context.md`).
...
- Как включить в конструкторе: добавьте чип SignalEngine из `GET /api/patterns` (не хардкодить десять id в `StrategyLab.tsx`). Таймфрейм и параметры паттерна задаются в `PatternSettingsModal`. Save вызывает `normalize_patterns`; тот же конфиг идёт в `strategy_backtest`, paper (`get_active_paper_strategy` → `StrategyEvaluator`) и live. Locked `test_20260731` не перезаписывать.
