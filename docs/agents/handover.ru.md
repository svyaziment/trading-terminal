...
Последнее обновление: 2026-09-07 (задача #143 — аналитический пакет устойчивости сетки трейлинг-стопа, §35). Сопутствующий файл: `project-context.ru.md` (английский оригинал: `project-context.md`).
...
## 35. Аналитика устойчивости сетки трейлинг-стопа на `test_20260830_new_level` (задача #143)
...
- Итог опубликованного прогона: 5 многоступенчатых сеток на тех же 3 305 кандидатах; паритет с #139
  подтверждён (0 расхождений по механике выхода; equity 103 216 ₽ против 103 176 ₽ в #139, PF 1.55,
  DD 2.72 п.п.). Разброс equity по сеткам — 12 742 ₽ (`two_step_aggressive` 108 569 ₽ — лучший,
  `three_step_steady` 95 827 ₽ — худший); `ref139` лидирует по сводному баллу устойчивости (47.3/100),
  и все пять сеток прибыльны в 9/9 walk-forward окон. Максимальный стресс (комиссия 0.15 % и
  20 б.п. проскальзывания) роняет базовую сетку до 16 708 ₽ без game-over; худшая сетка относительно
  контрольной (stakeholder-тест) — `three_step_steady` (Δequity −525 ₽). Согласованность выходов:
  идентичный исход у 80.5 % сделок (медианный парный ρ Спирмена 0.86, минимум 0.7497), причина
  выхода относительно базовой сетки меняется у 1 112 сделок из 3 305, материальных (порог 20 ₽) —
  1 057. Решения Product Owner нет; см. отчёт §4–§8.
- Воспроизводимость: `--stage report` пересобирает `report.md`, `summary.json` и `run.md` только из
  ...
  длительность). Полный `--stage all`: exit 0 — 156.73 с анализа (`summary.json.elapsed_sec`) внутри
  160.62 с процесса (`run.md`).
- Задокументированные ограничения (отчёт §13, держать честными): 5 многоступенчатых сеток вместо
  заказанных 8–12 (нет одношаговых и «безубыточного» шага), стресс мягче заказанного
  (комиссия 0,06 / 0,10 / 0,15 % и проскальзывание в б.п. вместо 0,3 / 0,6 / 1,5 % и шагов цены MOEX),
  нет чувствительности к `risk_reward`, нет графиков. Это долг #143, перечисленный в блоке
  продолжения отчёта — не расшишать решётку молча внутри этого пакета.
- Вывод: только аналитика. Composite-балл устойчивости не автономен: компоненты стресс-капитала,
  деградации DD и walk-forward в этом прогоне дают всем пяти сеткам одинаково (0.0 / 0.0 / 15.0),
  различают только абсолютная просадка и стабильность причины выхода (отчёт §4). Дефолтную сетку
  утверждает Product Owner (#144); работа над движком — #145, паритет боевого пути — #147,
  песочница — #151, приёмка — #152. Статус зеркалится в
  `project-context.ru.md` §18 и блок W дорожной карты (§8).
