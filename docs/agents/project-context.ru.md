...
Последнее обновление: 2026-08-30 (задача #137 автономный overnight LiveExecutor). Источник: docs/refresh/context_collector.py + git ls-files.
...
│ │ │ ├── trading_config.py # ЕДИНЫЙ ИСТОЧНИК ИСТИНЫ: вселенная, LIVE_UNIVERSE (задача #135, 9 имён PO; top-5 из #66 — история), стратегии, live risk policy, LEVEL_STATE_MACHINE (#106), LEVEL_BREAKOUT_RETEST (#107); трекер также для levels_sr_breakout (#117) и levels_sr_support (#127)
...
**Sandbox live execution** (`live_executor.py`, опциональный фоновый процесс): использует тот же `StrategyEvaluator` и live 1min контекст, затем требует окно входа MOEX [10:00, 19:00) МСК, свежий imbalance стакана, проверяет sandbox-баланс, рассчитывает целое число лотов, выставляет market BUY и записывает позицию в `trading.live_positions`. Overnight-запуск: `START_LIVE_EXECUTOR=1 ./start_processes.sh` (без `DURATION_MINUTES`) ждёт 10:00 МСК и останавливается в 19:00. Обычный paper-запуск не выставляет брокерские ордера.
...
- **Задача #137 (2026-08-30)**: overnight LiveExecutor. Без `DURATION_MINUTES` `start_processes.sh` поднимает paper до ближайшего будничного 19:00 МСК; LiveExecutor ждёт 10:00, входит только в [10:00, 19:00) и мониторит стоп/тейк до закрытия. Календарь: `MOEX_SESSION` / `moex_session.py`. Canary по-прежнему с явным `DURATION_MINUTES=N`.
...
- **Вселенная**: top-15 по PF (`trading_universe`) остаётся рейтингом paper через `get_trading_universe()`. Таблицу не сужать. Streaming и data refresh используют `get_streaming_universe()` = top-15 ∪ `LIVE_UNIVERSE`. Sandbox live execution использует PO-список задачи #135 `LIVE_UNIVERSE` = ROSN, IRAO, AFKS, NVTK, SBER, MTSS, PHOR, MOEX, FLOT через `get_live_trading_universe()` (без обрезки по top-15). Топ-5 из #66 (SBER, LKOH, RUAL, NVTK, GAZP) остаётся историческим в `analytics/issue-66-live-universe/`.
