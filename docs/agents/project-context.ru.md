...
Все брокерские вызовы проходят через token bucket с потолком 10 запросов/сек. SIGTERM/SIGINT только устанавливает флаг остановки; финальная очистка отменяет ожидающие entry/protection заявки, обновляет состояние БД и при включённом `close_positions_on_shutdown` закрывает открытые sandbox-позиции. Полную политику возвращает `get_live_trading_config()` из `trading_config.py`.
