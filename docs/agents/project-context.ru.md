...
### Паритет боевого пути трейлинг-стопа (задача #147, гейт эпика #142)
...
`2024-08-01` … `timestamp < 2026-08-21`, 28 тикеров, 50 000 ₽ / слот 10 000 ₽ / макс 5,
и снимает три книги: A_prod (трейлинг выключен), B_prod (сетка `ref139` инжектирована
явно), B_default (ступени из `trading_config.TRAILING_STOP` = `ultra_late_tight`).
Вердикт гейта: A_prod=PASS (equity 95179.91 против 95180.01 ₽,
n 2649, PF 1.41, daily MaxDD 6.49 п.п., причины выхода
совпадают, входы на уровне кандидатов 0/0);
B_prod=FAIL (equity 111458.96 ₽ > A, PF 1.5, daily MaxDD 4.01 п.п.
против 2.74 п.п. книги B #139 — полоса 1.0 п.п. не пройдена: структурная обратная связь
live-контура, ранний trailing-выход освобождает тикер и даёт +789
новых кандидатов против 3 305 оверлея при 15 пропущенных;
механика выхода бит-в-бит по `grid_check` #145, 0/3305 на обеих сетках);
B_default=PASS (equity 120753.7 ₽, PF 1.56, daily MaxDD 3.96 п.п.
против опубликованных 3.06 п.п. `ultra_late_tight` #143, сплит stop>trailing>take).
OVERALL=FAIL: формальный гейт не пройден по полосе daily MaxDD книги B; критерии B/D
пересказаны в `run.md` v3 ДО прогона, допуски к результату не подгонялись, вердикт
вынесен на подпись TL/PO (черновик комментария — в `report.md` пакета). Защищённые строки
126/36/102/118 не тронуты; записи в `paper_positions` / `backtest_results` не писались.
