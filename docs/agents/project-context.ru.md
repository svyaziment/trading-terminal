...
Последнее обновление: 2026-08-30 (задача #127 levels_sr_support — только поддержка с трекером). Источник: docs/refresh/context_collector.py + git ls-files.
...
│ │ │ ├── trading_config.py # ЕДИНЫЙ ИСТОЧНИК ИСТИНЫ: вселенная, live top-5, стратегии, live risk policy, LEVEL_STATE_MACHINE (#106), LEVEL_BREAKOUT_RETEST (#107); трекер также для levels_sr_breakout (#117) и levels_sr_support (#127)
...
| U | Поддержка с трекером (эпик #126) | #127 backend `levels_sr_support` готово; #128 чип Lab / #129 isolated vs #124 B-support / #130 портфель #44 в ожидании |
