...
| W | Ступенчатый трейлинг-стоп в боевом пути (эпик #142) | Planned — #144 контракт `config.trailing_stop`, #145 движок/плагин/портфельный симулятор, #146 редактор в Lab, #147 гейт паритета с #139, #143 — аналитика робастности готова (5 многоступенчатых сеток, walk-forward, стресс издержками — §18), #148 paper-контур, #149 API и фильтры, #150 панели Paper/Live, #151 песочница `LiveExecutor`, #152 приёмка на живом периоде (вердикт). Поставляется **выключенным по умолчанию**; основание — результат #139 (A 95 180.01 -> B 103 176.00 RUB, PF 1.41 -> 1.54, daily MaxDD 6.49% -> 2.74%) |
...
## 18. Аналитика устойчивости сетки трейлинг-стопа (задача #143)

`analytics/issue-143-trailing-robustness/` проигрывает книгу #139 (§17) off-engine на пяти сетках
ступенчатого трейла — `ref139` (`+2R→+1.5R, +2.5R→+2R`, консенсус #139) плюс `two_step_aggressive`,
`three_step_steady`, `tight_after_take` и `late_conservative` — на тех же 28 тикерах, locked-конфиге
id=126, 3 305 кандидата, капитал 50 000 ₽, слот 10 000 ₽, максимум 5 позиций. Входы считает
`StrategyEvaluator` по опубликованным 1m-путям, выход —
...
- Equity 95 826.80 … 108 568.88 ₽ (разброс 12 742.08 ₽ — больше, чем весь эффект трейлинга из #139,
  +7 995.99 ₽); PF 1.49 … 1.59; daily MaxDD 2.42 … 4.34 п.п.; GAME OVER нет.
- Стресс издержками (комиссия 0.06 / 0.10 / 0.15 % × проскальзывание 0 / 5 / 10 / 20 б.п., 60
  прогонов): в худшем узле все сетки убыточны (PnL −31 959 … −40 386 ₽) и ни одна не доходит до
  GAME OVER — equity падает до 9 613.65 … 18 041.44 ₽, просадка деградирует до +79.81 п.п. Издержки
  двигают книгу сильнее, чем выбор ступеней.
- Walk-forward (9 трёхмесячных окон, ≥20 сделок в окне): каждая сетка прибыльльна в 9/9 окон,
  худшее окно базовой сетки +745 ₽ — знак устойчив, уровень нет.
- Сводный балл устойчивости (`summary.json.grids[].robustness_score_0_100`) фактически держится на
  ...
  DD» в этом прогоне обнуляются у всех сеток, а walk-forward платит всем одинаковые 15 (9/9
  прибыльных окон), поэтому разброс узкий — `ref139` 47.3, `two_step_aggressive` 46.2,
  `three_step_steady` 46.0, `tight_after_take` 45.7, `late_conservative` 44.3. Балл — сводка, а не
  целевая функция: сетку под него не оптимизировали.
- Согласованность выходов: 80.5 % сделок дают одинаковый исход во всех пяти сетках (медианный ρ Спирмена
  0.86, минимум 0.7497). Причина выхода относительно базовой сетки меняется у 1 112 сделок из 3 305,
  материальных (порог 20 ₽) — 1 057, новой причиной чаще всего становится `trailing` (784 раз).
  Относительно контрольной `tight_after_take` — stakeholder-тест «худшая сетка
  существенно не ниже контрольной» — `three_step_steady` даёт −524.60 ₽.

Что не сделано и зафиксировано как долг #143 в `report.md` §13: 5 многоступенчатых сеток вместо
заказанных 8–12 (нет одношаговых и «безубыточной» `stop = 0`); стресс мягче заказанного (нет
шага цены MOEX и чувствительности к минимальному лоту); нет чувствительности к `risk_reward` 1:2;
не вычислен порог «фиксированный стоп против трейлинга» при максимальном стрессе (книга A в стресс
не прогонялась); графиков нет. Выбор дефолтной сетки в проде — решение Product Owner в #144, а не
результат этого пакета. Боевой путь не затронут: движок — #145, гейт паритета — #147.
