...
- `strategies`: id, name (unique), config (jsonb: patterns, confirm_windows, commission_pct, slippage_pct, risk_reward, trailing_stop, n_runs), in_paper_test (bool), locked (bool), description. `trailing_stop` — верхнеуровневый блок ступенчатого трейлинг-стопа из задачи #144 (§6, без миграции схемы, по умолчанию выключен); остальные ключи сохраняют смысл, который имели до #144.
