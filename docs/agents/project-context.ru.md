...
Последнее обновление: 2026-09-06 (эпик #142 — ступенчатый трейлинг-стоп запланирован в боевом пути выхода; roadmap Block W). Источник: docs/refresh/context_collector.py + git ls-files.
...
| W | Ступенчатый трейлинг-стоп в боевом пути (эпик #142) | Planned — #144 контракт `config.trailing_stop`, #145 движок/плагин/портфельный симулятор, #146 редактор в Lab, #147 гейт паритета с #139, #143 — аналитика робастности готова (решётка из 8 сеток `143-trailing-v3`, walk-forward, стресс издержками — §18; саму форму решётки допоставила #155), #148 paper-контур, #149 API и фильтры, #150 панели Paper/Live, #151 песочница `LiveExecutor`, #152 приёмка на живом периоде (вердикт). Поставляется **выключенным по умолчанию**; основание — результат #139 (A 95 180.01 -> B 103 176.00 RUB, PF 1.41 -> 1.54, daily MaxDD 6.49% -> 2.74%) |
...
решение Product Owner в #144, а не результат этого пакета. Боевой путь не затронут: движок — #145,
гейт паритета — #147.
