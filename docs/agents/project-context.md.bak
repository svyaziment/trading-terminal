...
**Issue #173 (2026-09-18)**: Live schema contract with fail-fast validation. `backend/app/analytics/live_schema.py` now owns the `trading.live_positions` shape required by the migration chain (`20260915_002` + `20260916_001`): 30 columns, a seven-status CHECK (`pending`, `open`, `closed_stop`, `closed_take`, `closed_trailing`, `closed_broker`, `cancelled`) and `trading.app_settings` with the `trailing_kill_switch` / `live_trailing_enabled` seeds. `ensure_live_positions_table()` delegates to `ensure_live_positions_schema()`, whose idempotent DDL is a superset of both migrations and also converges databases created by the historical 20-column runtime DDL. `LiveExecutor.initialize()` runs `assert_live_schema()` immediately after that DDL and aborts with `LiveSchemaError` (missing columns/statuses plus an `alembic upgrade head` hint), so drift is caught before the first order instead of mid-trade with `UndefinedColumn` or a CHECK violation. `live_executor_preflight.collect_preflight()` exposes the same contract as the `live_positions_schema` check with a `details.live_schema` block (`columns_found`, `missing_columns`, `statuses_found`, `missing_statuses`). Missing `app_settings` keys and unexpected extra columns are warnings, not blockers.

The take-profit is submitted immediately as a resting sell-limit. Since Issue #175 the stop-loss is a real broker `STOP_LOSS` stop order armed as soon as the entry fills (`broker_stop_id`); the historical synthetic path — wait until the broker's current price reaches the stop, cancel the take, submit a sell-limit at the observed price — remains only as a fallback while a position has no broker stop. Position reconciliation polls `get_positions()`; disappearance after a take or triggered stop closes the DB row, is classified from the authoritative stop-order status and is reconciled against the real fill from `get_operations()`. External SELL handling cancels protection (stop orders through `cancel_stop_order()`) and closes through a sandbox market order.

**Issue #174 (2026-09-19)**: Executor resilience and safe shutdown. The main `run()` loop is now protected against transient errors: `monitor_positions()` and `process_latest_bars()` calls are wrapped in `try/except` with a consecutive-error counter (`MAX_CONSECUTIVE_ERRORS = 5`); exceeding the threshold logs a critical alert and stops the executor gracefully. Errors are isolated at the unit-of-work level — per ticker in `process_latest_bars()` and `refresh_contexts()`, per position in `monitor_positions()` — so one failure does not break the rest. Safe shutdown: when `close_positions_on_shutdown=false` (the default), open positions are **not** unprotected — stop/take orders remain in place with the broker, and only pending entry orders are cancelled. Advisory lock `pg_try_advisory_lock(151001)` is now acquired on a **dedicated connection** held for the entire executor lifetime and released on the same connection in `shutdown()`, preventing lock leaks across pooled connections. Heartbeat and metrics: `heartbeat_ts`, `iterations_total`, `errors_total`, `errors_consecutive`, `last_error_at` are updated each iteration; `get_metrics()` returns the dictionary for external monitoring (task E #177). Tests: `test_live_executor.py` (78 tests, including 10 new for #174).

**Issue #175 (2026-09-22)**: Broker-side position protection, amend-trailing and fill reconciliation. `TinkoffSandboxClient` gained `post_stop_order()` (`PostStopOrder`: STOP_LOSS/TAKE_PROFIT, buy/sell, protective limit price, client idempotency key, `good_till_cancel`/`good_till_date`), `get_stop_orders()` (`GetStopOrders` with a status filter plus a client-side instrument filter, since the API has none), `cancel_stop_order()` (`CancelStopOrder`), `get_orders()` (`GetOrders`, resting orders) and `get_operations()` (`GetOperations`, executed operations with their per-trade fills); all of them reuse the existing retry/backoff policy and the sandbox-only guardrails. `LiveExecutor` arms a `STOP_LOSS` as soon as the entry fills — trigger aligned down to `min_price_increment`, limit `trailing_protective_ticks` ticks below the trigger — and stores the id in `broker_stop_id`. Every trailing ratchet amends by duplication (post the higher stop → confirm through `GetStopOrders` → cancel the older, lower one), so protection is never interrupted; when confirmation is inconclusive the superseded stop is cancelled later by `_cancel_pending_stops()` (on confirmation, or after `oco_check_delay_seconds` at the latest), which bounds the double-protection window. Exits are reconciled with real fills: `get_operations()` supplies `exit_price_actual` and `lots_executed` (shares → lots via `lot_size`, trade-weighted price when trades are present), the reason is classified from the authoritative stop status (`EXECUTED` → `closed_stop`/`closed_trailing`, still `ACTIVE` → `closed_take`), then from the fill price against the model levels, and finally by the legacy id heuristic (`closed_broker` when ambiguous); `slippage_bp` / `slippage_r` are recorded next to `exit_price_model`. OCO is emulated by monitoring: each close queues both legs and, after `oco_check_delay_seconds`, a surviving stop/take is cancelled with a critical `OCO monitoring:` alert (up to `oco_check_attempts` verification attempts). Two invariants are re-checked every `broker_stop_verify_interval_seconds` — no open position without `broker_stop_id`, and no stored `broker_stop_id` missing from the broker's active stop list — each violation logs a critical `Invariant violation:` and re-arms with exponential backoff, while arming failures log `protection_failed` and are exposed through `get_metrics()` (`stops_armed_total`, `stop_amend_total`, `stop_amend_failed_total`, `protection_failed_positions`, `invariant_violations_total`, `oco_orphans_cancelled_total`, `fills_reconciled_total`). The token bucket is prioritised `protection` > `trailing` > `entry` (entries must leave `entry_token_reserve` tokens). New `LIVE_TRADING` keys: `broker_stop_enabled`, `protection_retry_seconds`, `broker_stop_verify_interval_seconds`, `oco_check_delay_seconds`, `oco_check_attempts`, `operations_lookback_hours`, `entry_token_reserve`; the `live_positions` schema from #173 is unchanged. Tests: `test_live_executor.py` (94 tests, including 16 new for #175) and `test_tinkoff_sandbox.py` (28 tests, including 15 new for #175).



Every broker API attempt, including internal retries and account discovery, passes through a token bucket capped at 10 requests/second, with Issue #175 priority classes: `protection` (stop arming, amend, cancel, fill reconciliation) and `trailing` (monitoring) always proceed, while `entry` calls must leave `entry_token_reserve` tokens behind, so a saturated bucket defers new entries instead of protection. SIGTERM/SIGINT only sets a shutdown flag; final cleanup then cancels pending entry orders and broker stops, updates DB state, optionally flattens open sandbox positions when `close_positions_on_shutdown` is enabled, releases advisory lock, and closes the standalone DB pool. The complete policy is returned by `get_live_trading_config()` from `trading_config.py`.
