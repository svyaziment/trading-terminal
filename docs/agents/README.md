...
**Решение Product Owner от 2026-09-08: боевой дефолт сетки ступеней — `ultra_late_tight`** (`+2.0R→+1.9R`, `+2.5R→+2.4R`, `+3.0R→+2.9R`), выбранный по robustness-решётке #143 / #155 (110 434 ₽, PF 1.60, 3 162 сделки, daily MaxDD 3.06 п.п.; компромисс — балл устойчивости 46.3 против 47.3 у `ref139`). Это значение по умолчанию для `TRAILING_STOP.steps` в #144; `enabled` остаётся `false`. Сетка `ref139` (#139) выводится из прод-дефолтов и остаётся **якорем паритета** гейта #147. Полностью — `project-context.md` / `.ru.md` §18 и `handover.md` / `.ru.md` §35.
...
| #143 | Робастность: сетки ступеней, walk-forward по периодам, slippage/комиссии — **закрыто** (PR #154) | Vulpec (Analytics) | #145 |
| #155 | Расширение решётки #143 до `143-trailing-v3`: 8 сеток (две одношаговые, безубыточная граница и PO-проба), машиночитаемый `summary.json.lattice`, тесты `backend/tests/test_issue155_analysis.py` — **закрыто** (PR #156); дало основания для решения PO о боевой сетке | Vulpec (Analytics) | #143 |
...
Красные линии эпика: трейлинг выключен по умолчанию и не меняет поведение при `enabled=false`; ступени только в R от входа; запертые `test_20260830_new_level` (126) и референсные 36 / 102 / 118 не трогаем; пакет `analytics/issue-139-trailing-stop-new-level/` не рефакторим; выбор боевой сетки — исключительное решение Product Owner (принято 2026-09-08: `ultra_late_tight`, менять может только PO); документация — EN и RU в одном PR.
